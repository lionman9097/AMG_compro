<template>
  ...
  >
    ...
    <div
      class="absolute left-0 top-0 bottom-0 w-1 bg-cobalt-accent transition-opacity duration-200"
    ...
    <div class="px-3 sm:px-4">
      <div class="flex items-start justify-between gap-6">
        ...
        <div class="min-w-0 flex-1">
          ...
          <h3 class="font-display text-lg sm:text-xl text-zinc-900 mb-1.5 group-hover:text-cobalt-accent transition-colors duration-150 tracking-tight flex items-center gap-2">
            {{ product.name }}
            <ChevronDown
              class="w-4 h-4 text-zinc-400 group-hover:text-cobalt-accent transition-transform duration-200"
        ...
        <!-- Right: quick specs preview -->
        <div class="hidden md:grid grid-cols-3 gap-x-6 gap-y-1 shrink-0 text-right">
          <div>
            <dt class="text-[10px] text-zinc-400 uppercase tracking-widest font-medium mb-0.5">Density</dt>
            <dd class="text-[13px] font-medium text-zinc-800 font-mono">{{ product.specs.density }}</dd>
          </div>
          <div>
            <dt class="text-[10px] text-zinc-400 uppercase tracking-widest font-medium mb-0.5">Hardness</dt>
            <dd class="text-[13px] font-medium text-zinc-800 font-mono">{{ product.specs.hardness }}</dd>
          </div>
          <div>
            <dt class="text-[10px] text-zinc-400 uppercase tracking-widest font-medium mb-0.5">Tack-Free</dt>
            <dd class="text-[13px] font-medium text-zinc-800 font-mono">{{ product.specs.tackFreeTime }}</dd>
          </div>
        </div>
      </div>

      <!-- EXPANDABLE TECHNICAL DATA SHEET & SPECIFICATION DRAWER -->
      <div v-if="isExpanded" class="mt-6 pt-6 border-t border-zinc-200/80 space-y-6 text-sm text-zinc-700 animate-fadeIn cursor-default" @click.stop>
        <!-- Section 1: Characters & Features -->
        <div v-if="product.characters && product.characters.length" class="bg-zinc-50 p-4 rounded-xl border border-zinc-200/60">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- Application Areas -->
          <div v-if="product.applicationAreas && product.applicationAreas.length" class="bg-white p-4 rounded-xl border border-zinc-200">
            <h4 class="font-display font-semibold text-zinc-900 text-xs tracking-wider uppercase mb-2 flex items-center gap-2">
          ...
          <div class="bg-white p-4 rounded-xl border border-zinc-200 space-y-3">
            <h4 class="font-display font-semibold text-zinc-900 text-xs tracking-wider uppercase flex items-center gap-2">
              <Box class="w-4 h-4 text-cobalt-accent" />
              Color & Package Formats
            </h4>
            <div class="text-xs">
              <span class="text-zinc-400 uppercase font-mono text-[10px] tracking-wider block mb-1">Available Colors</span>
              <div class="flex flex-wrap gap-1.5">
                <span v-for="c in product.colors" :key="c" class="px-2 py-0.5 bg-zinc-100 border border-zinc-200 rounded text-zinc-800 text-[11px] font-medium">
                  {{ c }}
                </span>
              </div>
            </div>
            <div class="text-xs">
              ...
              <div class="flex flex-wrap gap-1.5">
                <span v-for="p in product.packaging" :key="p" class="px-2 py-0.5 bg-zinc-900 text-white rounded text-[11px] font-mono">
                  {{ p }}
                </span>
              </div>
            </div>
        ...
        <div v-if="product.storageSafety" class="bg-amber-50/50 p-4 rounded-xl border border-amber-200/80 text-amber-900 text-xs space-y-2">
          <h4 class="font-display font-semibold text-amber-950 text-xs tracking-wider uppercase flex items-center gap-2">
            <AlertTriangle class="w-4 h-4 text-amber-600" />
            Storage & Safety Attention
          </h4>
          <p><strong class="font-semibold">Storage Period:</strong> {{ product.storageSafety.storagePeriod }} — {{ product.storageSafety.storageConditions }}</p>
